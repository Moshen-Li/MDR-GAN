...
from model import ADNet, DnCNN
from model1 import Generator, PatchGAN, Generator1, Generator2, Generator3, UnetGenerator, PixelDiscriminator
...
def DNN_printer(model, input_size, batch_size=-1, device="cuda"):
    ...
    print("-----------------------------Author: Moshen Li---------------------------------")
    line_new = "{:>20} {:>20} {:>15} {:>13} {:>15}".format("Layer (type)", "Output Shape", "O-Size(MB)", "Param #",
                                                           "P-Size(MB)")
    print(line_new)
    print("==================================================================================")
    total_params = 0
...
if __name__ == "__main__":
    ...
    model = UnetGenerator(1,1,7).to(device)
    model2 = PixelDiscriminator(1).to(device)
    DNN_printer(model, input_size, batch_size=1, device="cuda")
    DNN_printer(model2, input_size, batch_size=1, device="cuda")
